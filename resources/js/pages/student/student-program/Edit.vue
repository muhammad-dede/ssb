...
const form = useForm({
    student_id: props.student_program?.student_id ?? "",
    program_code: props.student_program?.program_code ?? "",
    period_id: props.student_program?.period_id ?? "",
    amount: props.student_program?.billing?.amount ?? 0,
    due_date: props.student_program?.billing?.due_date ?? getDefaultDueDate(),
});
...
const submit = () => {
    form.patch(
        route("admin.student-program.update", props.student_program?.id),
        {
            preserveScroll: true,
        }
    );
...
const breadcrumbs = [
    ...
    { title: "Ubah", href: "/admin/student-program/edit" },
];
</script>

<template>
    <Head title="Ubah Registrasi" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <MainContent>
            <HeadingGroup>
                <Heading
                    title="Ubah Registrasi"
                    description="Formulir untuk mengubah data registrasi siswa yang telah terdaftar"
                />
            </HeadingGroup>
                        ...
                        <CardFooter>
                            <div class="space-x-2">
                                ...
                                <Link
                                    ...
                                    :href="route('admin.student-program.index')"
                                    :class="
